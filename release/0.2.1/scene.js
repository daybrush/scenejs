 ...
 */
  ...
  */
		...
		value: function calculateIterationTime() {
			...
			var activeTime = parseInt(Math.max(currentTime - delay, 0) * 10000, 10) / 10000;
		...
		value: function tick(now) {
			...
			var currentTime = this.getTime() + Math.min(1000, now - prevTime) / 1000;
 ...
 */
	...
	_createClass(SceneItem, [{
		...
		value: function setDuration(duration) {
			var ratio = duration / this.getDuration();
  ...
  */
		...
		value: function getNowValue(role, property, time) {
			...
			var ratio = this.state.easing((time - startTime) / (endTime - startTime));
...
var CSSItem = function (_SceneItem) {
	...
	_createClass(CSSItem, [{
		...
		value: function setCSS(time, properties) {
			...
			var element = this.options.element && this.options.element[0];
  ...
  frame.getProperty("opacity"); // 0.5
  	*/
		...
		value: function toCSS() {
			...
			var easing = options.easingName || this.state.easingName;
			var fillMode = options.fillMode || this.state.fillMode;
			var count = options.iterationCount || this.state.iterationCount;
			var cssArray = [];
 ...
 */
	...
	_createClass(Scene, [{
		...
		value: function setDuration(duration) {
			var items = this.items;
			var sceneDuration = this.getActiveDuration();
			var ratio = duration / sceneDuration;

			for (var id in items) {
				var item = items[id];
				var _item$state = item.state,
				    playSpeed = _item$state.playSpeed,
				    delay = _item$state.delay;

				var time = item.getTotalDuration() / playSpeed * ratio - delay / playSpeed;
