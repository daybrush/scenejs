...
var _Animator2 = __webpack_require__(2);
		...
		value: function load() {
			...
			for (var time in properties) {
				...
				if (time === "from") {
					...
					realTime = duration;
				} else if ((0, _utils.isPercent)(time)) {
...
var _PropertyObject = __webpack_require__(4);
  	...
  	*/
		...
		value: function toKeyframes() {
			...
			var keyframes = times.map(function (time) {
				...
				return time / duration * 100 + "%{" + frame.toCSS() + "}";
			});
		...
		value: function toCSS() {
			...
			var playSpeed = (options.playSpeed || 1) * this.state.playSpeed;
			var delay = ((options.delay || 0) + this.state.delay) / playSpeed;
